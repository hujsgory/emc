...
static inline double cost(double dx,double dy){
...
static PyObject * _smn(PyObject * self, PyObject * args){
	...
	Py_ssize_t len_list1 = PyList_GET_SIZE(list1);
	Py_ssize_t len_list2 = PyList_GET_SIZE(list2);
	int m=0;
    for (Py_ssize_t idx_list1=0; idx_list1<len_list1; idx_list1++){
        ...
        int len_bound_m=(int)PyLong_AsLong((PyObject *)PyDict_GetItemString(bound_m,"n_subint"));
        for(int i=0; i<len_bound_m; i++){
            ...
            for (Py_ssize_t idx_list2=0; idx_list2<len_list2; idx_list2++){
                ...
                for (int j=0;j<len_bound_n;j++){
					...
					double xn  = center(subs_j_begx,subs_j_endx),yn=center(subs_j_begy,subs_j_endy);
